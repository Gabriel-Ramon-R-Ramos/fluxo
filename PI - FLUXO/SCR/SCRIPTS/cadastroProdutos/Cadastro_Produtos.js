...
// Função para atualizar o breadcrumb com a aba ativa
function atualizarBreadcrumb(nomeAba = 'Dados Gerais') {
  // Encontra o elemento ul dentro do breadcrumb
  const breadcrumbList = document.querySelector('.breadcrumb ul');

  if (!breadcrumbList) return;

  // Remove qualquer terceiro item que possa existir (para evitar duplicatas)
  const terceiroItem = breadcrumbList.querySelector('li:nth-child(3)');
  if (terceiroItem) {
    terceiroItem.remove();
  }

  // Cria um novo elemento li para o nome da aba atual
  const novoItem = document.createElement('li');
  novoItem.textContent = nomeAba;
  novoItem.classList.add('current-page');

  // Adiciona o novo elemento à lista
  breadcrumbList.appendChild(novoItem);
}
...
document.addEventListener('DOMContentLoaded', () => {
  atualizarBreadcrumb('Dados Gerais'); // Atualiza o breadcrumb com a aba inicial
