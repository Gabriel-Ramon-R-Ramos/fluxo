...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateIndicator(button) {
    ...
    const offsetLeft =
      buttonRect.left - containerRect.left + filterNav.scrollLeft;
  ...
  async function fetchLancamentos(page = 0, size = 10) {
    try {
      const response = await fetch(
        `https://api-fluxo.onrender.com/lote/movimentacoes?page=${page}&size=${size}`,
        {
          method: "GET",
          headers: {
        ...
        <tr data-id="${item.id}">
          ...
          <td>${
            item.movementType === "ENTRADA"
              ? item.movementAmount.toFixed(2)
              : "0,00"
          }</td>
          <td>${
            item.movementType === "SAIDA"
              ? item.movementAmount.toFixed(2)
              : "0,00"
          }</td>
  ...
  function inicializarMenuOpcoes() {
    ...
    document.querySelectorAll(".menu-acao li").forEach((item) => {
      item.addEventListener("click", function (e) {
        ...
        const lancamentoId = tr.getAttribute("data-id");
  ...
  document.querySelector(".btn-salvar").addEventListener("click", async (e) => {
    ...
    const tipo = document.getElementById("tipo").value.toUpperCase(); // 'entrada' ou 'saida'
    const produto = document.getElementById("produto").value;
    const quantidade =
      parseInt(document.getElementById("quantidade").value) || 0;
    const preco = parseFloat(document.getElementById("preco").value) || 0;
    const lote = document.getElementById("lote").value || "";
    const localizacaoLote =
      document.getElementById("localicaoLote").value || "";
